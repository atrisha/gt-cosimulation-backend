...
def count_agents_crosswalk_dataset():
    count_map = dict()
    #for file_id in ['591','592','593','595','596','598','599','600','602','603','604','605','606','607','609','610','611']:
    for file_id in constants.ALL_FILE_IDS:
        print('processing',file_id)
        constants.CURRENT_FILE_ID = file_id
        #conn = sqlite3.connect('D:\\crosswalk-dataset\\crosswalk_'+constants.CURRENT_FILE_ID+'.db')
        conn = sqlite3.connect('D:\\intersections_dataset\\dataset\\'+constants.CURRENT_FILE_ID+'\\uni_weber_'+constants.CURRENT_FILE_ID+'.db')
        c = conn.cursor()
        q_string = "select type,count(*) from TRACKS group by type"
        c.execute(q_string)
        res = c.fetchall()
        for row in res:
...
if __name__ == '__main__':     
    count_agents_crosswalk_dataset()
